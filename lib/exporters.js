...
async function createPptBuffer({ dataset, columns, rows, charts }) {
  ...
  cover.addText(
    `This export contains the full table plus ${charts.length} chart visual${charts.length === 1 ? "" : "s"}.`,
    {
      x: 0.6,
      y: 1.85,
      w: 8.8,
  ...
  const columnChunks = chunkArray(columns, 8);
  const rowChunks = chunkArray(rows, 14);

  columnChunks.forEach((columnChunk, columnChunkIndex) => {
    rowChunks.forEach((rowChunk, rowChunkIndex) => {
      const slide = pptx.addSlide();
      slide.background = { color: "091223" };
      slide.addText(
        `Data table ${columnChunkIndex + 1}/${columnChunks.length} · rows ${rowChunkIndex * 14 + 1}-${rowChunkIndex * 14 + rowChunk.length}`,
        {
          x: 0.5,
          y: 0.4,
          w: 12,
      ...
      slide.addTable(
        [
          columnChunk,
          ...rowChunk.map((row) =>
            columnChunk.map((column) => formatCellValue(row[column])),
          ),
        ],
        {
          x: 0.35,
          ...
          margin: 0.04,
          autoFit: false,
          colW: Array.from({ length: columnChunk.length }, () => 1.55),
          rowH: 0.32,
        },
      );
    });
