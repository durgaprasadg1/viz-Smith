...
function buildFallbackRelationships(columnProfiles, maxRelationships) {
  ...
  // Build each chart type's candidates into its own bucket first, instead of
  // one big array. This lets us interleave types round-robin below so the
  // final slice(0, maxRelationships) doesn't always chop off the same
  // types (previously scatter/histogram/pie were always last and always
  // the ones dropped once candidates exceeded the budget).
  ...
  const histogramCandidates = [];
  numeric.forEach((numericColumn) => {
    histogramCandidates.push({
  ...
  // Round-robin merge: take one candidate from each non-empty bucket in
  // turn (line, bar, scatter, histogram, pie), repeating until every
  // bucket is exhausted. This spreads the eventual budget cut evenly
  // across chart types instead of always favoring whichever type was
  // built first.
  const buckets = [
    lineCandidates,
    barCandidates,
  ...
  const relationships = [];
  let remaining = buckets.some((bucket) => bucket.length > 0);
  let cursor = 0;
  while (remaining) {
    remaining = false;
    buckets.forEach((bucket) => {
      if (cursor < bucket.length) {
        relationships.push(bucket[cursor]);
        remaining = true;
      }
    });
    cursor += 1;
  ...
  const limited = relationships.slice(0, maxRelationships);
  limited.forEach((relationship, index) => {
    relationship.priority = index + 1;
  });
...
- Prefer pie/donut when a single column is categorical with <= 8 meaningful categories.
Dataset metadata:
...
async function getGrokChartSuggestions(payload, maxRelationships) {
  // console.log("AI LGNE WLALA hiai ")
