...
export function buildPreparedCharts(relationships, rows, columns) {
  ...
  safeRelationships.forEach((relationship, index) => {
    ...
    if ((chartType === "bar" || chartType === "horizontalBar") && chartData.length) {
      const uniqueCategories = chartData.length;
      const totalValue = chartData.reduce((s, item) => s + Number(item?.value || 0), 0);
      const allNonNegative = chartData.every((item) => Number(item?.value) >= 0);


      if (!secondary && uniqueCategories > 0 && uniqueCategories <= 8 && allNonNegative && totalValue > 0) {
        renderType = "pie";
      }
    }
